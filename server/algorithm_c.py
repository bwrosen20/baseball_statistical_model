...
with app.app_context():
    ...
    url_ending = rows[len(games_that_year)-299  ].select('a')[2].get("href")
    ...
    for game in rows[(len(games_that_year)-299):]:
    ...
    test_range= [i/100 for i in range(25,51,5)]
    ...
    for test_value in test_range:
        ...
        sorted_players = [player["object"] for player in sorted_by_value_bets]
        ...
        print(f"Value: {test_value}, Total: {team_total}")
        test_value_dict = {"value":test_value,"team_total": team_total}

        test_values.append(test_value_dict)

    sorted_test_values = sorted(test_values,key=itemgetter('team_total'))
    sorted_test_values.reverse()
    for item in sorted_test_values:
        value = item["value"]
        team_total = item["team_total"]
        print(f'{value} Team Total: {team_total}')

    

    #test guys
        #pitcher stdev
        #percentage of 2nd stdev (if difference is within .1)
